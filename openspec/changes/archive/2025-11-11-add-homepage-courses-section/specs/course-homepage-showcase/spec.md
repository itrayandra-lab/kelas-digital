# Spec: Course Homepage Showcase

**Capability**: Display featured courses in a dedicated homepage section to increase course visibility and drive enrollments.

**Related Specs**:
- Modifies: `openspec/specs/homepage-sections/spec.md` (adds 8th section)
- Future: May integrate with course popularity tracking (not yet implemented)
...
**Files Modified**:
- `app/Http/Controllers/HomeController.php` - Add featured courses query
- `app/Models/Course.php` - Add featured scope and fillable fields
- `resources/views/home.blade.php` - Add courses section view
- `database/migrations/2025_11_11_000001_add_featured_fields_to_courses_table.php` - New migration

**Files Referenced**:
- `docs/STYLE_GUIDE.md` - Design system patterns
- `openspec/specs/homepage-sections/spec.md` - Homepage structure (updated)
- `routes/web.php` - Course routes (no changes needed)

**Related Models**:
- `Course` - Primary model
- `CourseCategory` - Related via belongsTo
- `Enrollment` - Related via hasMany
- `User` - Related via enrollments pivot

**Related Routes**:
- `GET /` - Homepage (HomeController@index)
- `GET /courses` - Course index (CourseController@index)
- `GET /courses/{slug}` - Course detail (CourseController@show)
