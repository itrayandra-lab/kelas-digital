...
class RoleService
{
    ...
    public function getPermissionGroups(): array
    {
        ...
        return [
            'Course Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'courses') &&
                ! str_contains($p->name, 'enroll') &&
                ! str_contains($p->name, 'enrolled')
            ),
            'Lesson Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'lessons')
            ),
            'Article Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'articles')
            ),
            'User Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'users') || str_contains($p->name, 'roles')
            ),
            'Enrollment Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'enrollments')
            ),
            'Category Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'categories')
            ),
            'Tag Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'tags')
            ),
            'Admin Panel' => $permissions->filter(fn ($p) => str_contains($p->name, 'admin') ||
                str_contains($p->name, 'dashboard') ||
                str_contains($p->name, 'reports') ||
                str_contains($p->name, 'site settings')
            ),
            'System Management' => $permissions->filter(fn ($p) => str_contains($p->name, 'manage roles') ||
                str_contains($p->name, 'manage permissions')
            ),
            'Student Features' => $permissions->filter(fn ($p) => str_contains($p->name, 'enroll') ||
                str_contains($p->name, 'enrolled') ||
